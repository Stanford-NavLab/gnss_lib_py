...
def test_find_elaz():
    Rx = np.reshape(np.array([consts.A, 0, 0]), [1, 3])
    test_XYZ = np.zeros([6, 3])
    test_XYZ[0,0] = 1.25*consts.A
    test_XYZ[1,0] = 0.75*consts.A
    test_XYZ[2,0] = consts.A; test_XYZ[2,1]= consts.A
    test_XYZ[3,0] = consts.A; test_XYZ[3,1]= -consts.A
    test_XYZ[4,0] = consts.A; test_XYZ[4,2]= consts.A
    test_XYZ[5,0] = consts.A; test_XYZ[5,2]= -consts.A
...
def test_measures_value_range():
    ...
    assert np.all(np.abs(satXYZV['x']).values < consts.A + 2e7), "Invalid range of ECEF x for satellite position"
    assert np.all(np.abs(satXYZV['y']).values < consts.A + 2e7), "Invalid range of ECEF y for satellite position"
    assert np.all(np.abs(satXYZV['z']).values < consts.A + 2e7), "Invalid range of ECEF z for satellite position"
