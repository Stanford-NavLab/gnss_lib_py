...
def fixture_load_clkdata_gps(clk_path):
    ...
    ----------      
    """
    clkdata_gps = parse_clk(sp3_path, constellation = 'G')
...
def fixture_load_clkdata_glonass(clk_path):
    ...
    ----------      
    """
    clkdata_glonass = parse_clk(clk_path, constellation = 'R')

    return clkdata_glonass

def test_sp3_eph(navdata_gps):
...
def test_gps_func(sp3data_gps, clkdata_gps): 
    ...
    for prn in range(1, NUMSATS_GPS+1):
        for sidx in range(len(sp3data_gps.t)):
            func_satXYZ = extract_sp3_func(sp3data_gps[prn], sidx, ipos = 10, method='CubicSpline')
            cxTime = sp3data_gps[prn].t[sidx]
            satXYZ_sp3, _ = compute_sp3_snapshot(func_satXYZ, cxTime, hstep = 1e-5, method='CubicSpline')
            
            satXYZ_sp3_exact = np.array([ sp3data_gps[prn].x[sidx], \
                                          sp3data_gps[prn].y[sidx], \
                                          sp3data_gps[prn].z[sidx] ])
            assert np.linalg.norm(satXYZ_sp3-satXYZ_sp3_exact)

        for sidx in range(len(clkdata_gps.t)):
            func_satT = extract_clk_func(clkdata_gps[prn], sidx, ipos = 10, method='CubicSpline')
            cxTime = clkdata_gps[prn].t[sidx]
            satBias_clk, _ = compute_clk_snapshot(func_satT, cxTime, hstep = 1e-5, method='CubicSpline') 
            assert np.linalg.norm(satBias_clk - clkdata_gps[prn].CLKbias[sidx])
...
def test_glonass_func(sp3data_glonass, clkdata_glonass): 
    ...
    for prn in range(1, NUMSATS_GLONASS + 1):
        for sidx in range(len(sp3data_glonass.t)):
            func_satXYZ = extract_sp3_func(sp3data_glonass[prn], sidx, ipos = 10, method='CubicSpline')
            cxTime = sp3data_glonass[prn].t[sidx]
            satXYZ_sp3, _ = compute_sp3_snapshot(func_satXYZ, cxTime, hstep = 1e-5, method='CubicSpline')
            
            satXYZ_sp3_exact = np.array([ sp3data_glonass[prn].x[sidx], \
                                          sp3data_glonass[prn].y[sidx], \
                                          sp3data_glonass[prn].z[sidx] ])
            assert np.linalg.norm(satXYZ_sp3-satXYZ_sp3_exact)

        for sidx in range(len(clkdata_glonass.t)):
            func_satT = extract_clk_func(clkdata_glonass[prn], sidx, ipos = 10, method='CubicSpline')
            cxTime = clkdata_glonass[prn].t[sidx]
            satBias_clk, _ = compute_clk_snapshot(func_satT, cxTime, hstep = 1e-5, method='CubicSpline') 
            assert np.linalg.norm(satBias_clk - clkdata_glonass[prn].CLKbias[sidx])
